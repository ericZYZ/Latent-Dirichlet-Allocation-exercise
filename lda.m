...
for iter = 1:100     % This can take a couple of minutes to run
  ...
  topic_post(:,iter)=(sk+alpha)/sum(sk+alpha);
  topic_1000(:, iter)=(skd(:,1000)+alpha)/sum(skd(:,1000)+alpha);
  
  for i=1:K
    p_wnd = (gamma+swk(:,i))/(sum(gamma+swk(:,i)));
    entropy(i,iter)= -sum(p_wnd.*log(p_wnd));
  end
  
end

figure
plot(topic_post','LineWidth',2);
xlim([1 20]);
legend('k=1','k=2','k=3','k=4','k=5','k=6','k=7','k=8','k=9','k=10','k=11','k=12','k=13','k=14','k=15','k=16','k=17','k=18','k=19','k=20');
title('Topic posteriors','FontSize',12);  
