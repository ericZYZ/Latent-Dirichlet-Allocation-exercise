...
for iter = 1:100     % number of Gibbs sweeps
  for d = 1:D       % for each document iterate through all its words
    ...
    for k = 1:K
      ll = c'*( log(swk(w,k)+gamma) - log(sk_words(k) + gamma*W) ); %log likelihood of words
  ...
  mixing_prop(:,iter)=(sk_docs+alpha)/(alpha*20+sum(sk_docs));
end

figure
plot(mixing_prop','LineWidth',2);
xlim([1 20]);
legend('k=1','k=2','k=3','k=4','k=5','k=6','k=7','k=8','k=9','k=10','k=11','k=12','k=13','k=14','k=15','k=16','k=17','k=18','k=19','k=20');
title('Evolution of the mixing proportions','FontSize',12);  
...
perplexity_bmm = exp(-lp/nd);   % perplexity
